...
# @author:aiden
# 追踪拾取
# Note: We only update some of he parameters. No major changes made to the script.
import os
import sys
import cv2
import math
...
start_place = False
target_color = ""
# Keep the robot slower near the cube to reduce overshoot and odom drift
linear_base_speed = 0.003
angular_base_speed = 0.02
...
# Fallbacks so we don't get stuck aligning forever
FORCE_PICK_TIMEOUT = 12.0          # seconds since start_pick
FORCE_PICK_LOST_TIMEOUT = 5.0      # seconds since last detection
...
def start_pick_callback(msg):
    global start_pick, yaw_angle, stop_y, stop, status, target_color, broadcast_status, last_seen_time, pick_start_time
    ...
    angular_pid.clear()
    last_seen_time = None
    pick_start_time = rospy.Time.now()
    start_pick = True
...
def pick_handle(usb_cam_img):
    global pick, count_turn, count_stop, angular_speed, linear_speed, status, d_x, d_y, broadcast_status, last_seen_time
    ...
    if not pick or debug:
        ...
        if debug:
            ...
            # If we are already centered and facing mostly straight, mark pick to avoid endless alignment
            if abs(object_center_x - stop_x) <= d_x and abs(object_center_y - stop_y) <= d_y and abs(object_angle) <= 10:
                pick = True
                mecnum_pub.publish(Twist())
                return usb_cam_img
            if status != "align":
        ...
        # Force pick if we've seen the target but spent too long aligning
        if not pick and last_seen_time and (now - last_seen_time).to_sec() > FORCE_PICK_LOST_TIMEOUT:
            pick = True
            mecnum_pub.publish(Twist())
        # Force pick if overall timer since start_pick is exceeded, even if target lost
        if not pick and pick_start_time and (now - pick_start_time).to_sec() > FORCE_PICK_TIMEOUT:
            pick = True
            mecnum_pub.publish(Twist())
